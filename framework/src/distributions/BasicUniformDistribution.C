/****************************************************************/
/*               DO NOT MODIFY THIS HEADER                      */
/* MOOSE - Multiphysics Object Oriented Simulation Environment  */
/*                                                              */
/*           (c) 2010 Battelle Energy Alliance, LLC             */
/*                   ALL RIGHTS RESERVED                        */
/*                                                              */
/*          Prepared by Battelle Energy Alliance, LLC           */
/*            Under Contract No. DE-AC07-05ID14517              */
/*            With the U. S. Department of Energy               */
/*                                                              */
/*            See COPYRIGHT for full restrictions               */
/****************************************************************/

#include "BasicUniformDistribution.h"

template<>
InputParameters validParams<BasicUniformDistribution>()
{
  InputParameters params = validParams<Distribution>();
  params.addParam<Real>("lower_bound", 0.0, "Distribution lower bound");
  params.addParam<Real>("upper_bound", 1.0, "Distribution upper bound");
  return params;
}

BasicUniformDistribution::BasicUniformDistribution(const InputParameters & parameters)
  : Distribution(parameters),
    _lower_bound(getParam<Real>("lower_bound")),
    _upper_bound(getParam<Real>("upper_bound"))
{
  if (_lower_bound >= _upper_bound)
    mooseError("The lower bound provided by the user is actually larger than the upper bound for given uniform distribution");
}

BasicUniformDistribution::~BasicUniformDistribution()
{
}

Real
BasicUniformDistribution::pdf(Real x)
{
  if (x < _lower_bound || x > _upper_bound)
    return 0.0;
  else
    return 1.0 / (_upper_bound - _lower_bound);
}

Real
BasicUniformDistribution::cdf(Real x)
{
  if (x < _lower_bound)
    return 0.0;
  else if (x > _upper_bound)
    return 1.0;
  else
    return (x - _lower_bound) / (_upper_bound - _lower_bound);
}

Real
BasicUniformDistribution::inverseCdf(Real y)
{
  if (y < 0 || y > 1)
    mooseError("The cdf value should be within [0,1], but ", y, " is provided");
  else
    return y * (_upper_bound - _lower_bound) + _lower_bound;
}

