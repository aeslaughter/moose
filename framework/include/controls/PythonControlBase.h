...
protected:
  ...
  /// The name of the sparameter to control
...
PythonControlBase<T>::PythonControlBase(const InputParameters & parameters) :
    ...
    _parameter(getParam<std::string>("parameter"))
{
...
PythonControlBase<T>::execute()
{
  ...
  PyObject* args = buildPythonArguments();
  if (args == NULL)
  ...
  // Convert the output to the correct type and update the parameter value
  setControllableValueByName<T>(_parameter, getPythonResult(result));
