...
class RenderAutoLink(core.RenderLink):
    ...
    def createHTML(self, token, parent):
        ...
        href = tag['href']
        if href.endswith('.md'):
            obj = token.root.page.findall(href, maxcount=1)
            if obj:
                tag['href'] = os.path.relpath(obj[0].local, os.path.dirname(token.root.page.local)).replace('.md', '.html') #TODO: extensions should not be hardcoded (see page.MarkdownNode)
        return tag
