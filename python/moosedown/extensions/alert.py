...
class AlertCommandBase(command.BlockCommand):
    ...
    def createToken(self, match, parent):
        ...
        if title:
            ...
            self.reader.lexer.tokenize(title_token, grammer, title, match.node, match.line)

        content = AlertContent(alert)
        grammer = self.reader.lexer.grammer('block')
        self.reader.lexer.tokenize(content, grammer, match['content'], match.node, match.line)
        return alert
