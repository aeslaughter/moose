...
class PETScConfigController(AutotoolsConfigController):
    ...
    def validObjectParams():
        ...
        params.add('major', vtype=int,
                   ...
                   user_data=AutotoolsConfigItem('LIBMESH_DETECTED_PETSC_VERSION_MAJOR', '0', int))
    ...
    def execute(self, obj, params):
        ...
        sys_version = packaging.version.parse(f"{major}.{minor}.{subminor}")
        ...
        if (the_version is not None) and (packaging.version.parse(the_version) != sys_version):
            self.skip('PETSc {} != {}', the_version, sys_version)
            self.debug(
