...
class TestMooseDocsNode(unittest.TestCase):
    ...
    def testNameOnly(self):
        ...
        self.assertEqual(node.path(), './only')
    ...
    def testExceptions(self):
        ...
        with self.assertRaises(mooseutils.MooseException) as e:
            node = MooseDocsNode()
        self.assertEqual(str(e.exception), 'The "name" string must be supplied to the MooseDocsNode object.')

        with self.assertRaises(mooseutils.MooseException) as e:
            node = MooseDocsNode(name=42)
        self.assertEqual(str(e.exception), 'The "name" string must be supplied to the MooseDocsNode object.')

        for site_dir in [None, 42, 'not/a/valid/directory']:
            with self.assertRaises(mooseutils.MooseException) as e:
                node = MooseDocsNode(name="foo", site_dir=site_dir)
            self.assertEqual(str(e.exception), 'The "site_dir" must be a string and a valid directory.')
...
class MarkdownNodeTest(unittest.TestCase):
    ...
    def testSingle(self):
        node = MarkdownNode(name='single', site_dir='site', parser=self._parser, markdown='content/index.md')
    ...
    def testBuildTree(self):
        ...
        """
        root = MarkdownNode(name='', site_dir='site', parser=self._parser, markdown='content/index.md')
