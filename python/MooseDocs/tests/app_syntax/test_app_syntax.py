...
class TestAppSyntax(MarkdownTestCase):
    ...
    def testParametersNonUnique(self):
        md = '!syntax parameters /BCs/Pressure'
        self.convert(md)
        self.assertInLogError('The syntax provided is not unique')
        self.assertInLogError('syntax parameters /BCs/Pressure')
    ...
    def testParametersSyntaxError(self):
        ...
        self.convert(md)
        self.assertInLogError('Failed to locate MooseObject or Action')
    ...
    def testDescriptionNotUnique(self):
        md = '!syntax description /BCs/Pressure'
        self.convert(md)
        self.assertInLogError('The syntax provided is not unique')
        self.assertInLogError('syntax description /BCs/Pressure')

    def testDescriptionDisableObjects(self):
        md = '!syntax description /BCs/Pressure objects=False'
        html = self.convert(md)
        self.assertIn("syntax is not associated with a specific MooseObject or Action", html)
    ...
    def testObjects(self):
        ...
        gold = '<a href="{}/docs/content/documentation/systems/Adaptivity' \
               '/Markers/framework/BoxMarker.md">BoxMarker</a>'.format(MooseDocs.ROOT_DIR)
    ...
    def testActions(self):
        ...
        html = self.convert(md)
        self.assertIn('<h2>Available Actions</h2>', html)

        msg = 'href="{}/docs/content/documentation/systems/Adaptivity/Markers/framework/' \
              'AddElementalFieldAction.md">AddElementalFieldAction</a>'.format(MooseDocs.ROOT_DIR)
    ...
    def testSubSystems(self):
        ...
        self.assertIn('<div class="collapsible-header moose-group-header">Framework Systems</div>',
                      html)
        gold = 'href="{}/docs/content/documentation/systems/' \
               'Adaptivity/Indicators/index.md">Indicators</a>'.format(MooseDocs.ROOT_DIR)
        self.assertIn(gold, html)
