...
class MooseObjectInfo(MooseInfoBase):
    ...
    def check(self):
        """
        Check that the MooseObject documentation if correct.
        """
...
class MooseApplicationSyntax(object):
    ...
    def getAction(self, name):
        """
        Return the action info object for given name.
        """
        return self._actions[name]

    def actions(self, prefix=None, include_self=True):
        """
        Return a list of all action info objects.
        """
    ...
    def getObject(self, name):
        """
        Return the info object for a given name.
        """
    ...
    def __getHelper(items, prefix=None, include_self=None):
        """
        Builds a list of info objects from the supplied item dict().
        """
        if prefix:
            out = []
