 ...
 */
bool isWithinSegment(const Point & segment1, const Point & segment2, const Point & point);
 ...
 * @param active_neighbor_children Temproary vector for use in the search
 */
                                    ...
                                    std::vector<NeighborInfo> & info)
{
  ...
  else if (T_candidate->level() < elem->level())
  {
    ...
    {
      mooseAssert(v1_edge == v2_edge, "Verticies should be contained in same edge");
...
/**
 * Find the child of a elem that contains a point on a specified side of elem.
 ...
 * @param v0 Vertex index on elem that is the zeroth vertex of the tringular face
 * @param v1 Vertex index on elem that is the first vertex of the tringular face
 * @param v2 Vertex index on elem that is the second vertex of the tringular face
 ...
 */
template <typename T>
bool withinEdgeTempl(const Elem * elem, const Point & point, ElemExtrema & extrema);
 ...
 * @return If the point is within an edge of the element
 */
bool withinEdge(const Elem * elem, const Point & point, ElemExtrema & extrema);
