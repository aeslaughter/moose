...
public:
  ...
  /**
   * Does not utilize standard element loops, so there is nothing to do here
   */
  void subdomainSetup() override final {}
  ...
  {
    return _parallel_ray_study->acquireParallelData(tid, std::forward<Args>(args)...);
  }
...
private:
  ...
  /// Debug Ray bank - only filled when out of opt modes
