...
TraceRay::exitsElem(const Elem * elem,
                    ...
                    const unsigned short skip_side,
                    ...
                    const Point * normals)
{
  ...
  if (skip_side != RayTracingCommon::invalid_side)
    traceAssert(_study.sideIsIncoming(elem, skip_side, (*_current_ray)->direction(), _tid),
                "Skip side is non-entrant");
                    ...
                    const Point * normals)
{
  ...
  // So that we can potentially change this and rerun in the while loop
  bool backface_culling = _backface_culling;
  const auto & direction = (*_current_ray)->direction();

  // TODO: Use the skip_side as a last resort
  while (true) // So that we can try again without culling
  {
    debugRay("  Entering with backface_culling = ", backface_culling);

    for (MooseIndex(T::num_sides) s = 0; s < T::num_sides; ++s)
    {
      debugRay("  Side ", s, " with centroid ", sidePtrHelper(elem, s)->centroid());

      // Don't search backwards
      if (s == skip_side)
      {
        debugRay("    Skipping due to not searching backwards");
        continue;
      }
      ...
      if (backface_culling)
      {
        if (normals[s] * direction < -TRACE_TOLERANCE)
        {
      ...
      // Backface culling disabled... but we have normals available - culling may have failed!
      else if (_backface_culling)
      {
        // We already checked this side
        if (normals[s] * direction >= -TRACE_TOLERANCE)
          continue;
...
#ifndef NDEBUG
        const auto side_elem = sidePtrHelper(elem, s);

        // Only validate intersections if the side is planar
        if (side_elem->has_affine_map())
        {
          if (current_intersected_extrema.atExtrema())
          {
            if (!current_intersected_extrema.isValid(elem, current_intersection_point))
              failTrace("Extrema is incorrect");
          }
          else if (!side_elem->contains_point(current_intersection_point))
    ...
    // Didn't find an intersection, but we used backface culling. Try again without it
    if (backface_culling)
    {
                    ...
                    const Point *)
{
  ...
  if (skip_side != RayTracingCommon::invalid_side)
  {
    intersected_side = (skip_side == 1 ? 0 : 1);
    ...
    debugRay("  Skip side is set to ", skip_side, " so setting to other side");
                               ...
                               unsigned short & best_elem_incoming_side)
{
  ...
  traceAssert(_intersection_distance == RayTracingCommon::invalid_distance,
              "Disatnce should be invalid");

  // Marker for the longest ray segment distance we've found in a neighbor. Start a quite
  // small number because at this point we're desperate and will take anything!
  Real longest_distance = 1.0e-12;
  // Temproraries for the intersection checks
...
TraceRay::applyOnInternalBoundary(const std::shared_ptr<Ray> & ray)
{
  ...
  if (_dim != 1 && _intersected_extrema.atExtrema())
  {
    ...
    for (const auto & neighbor_info : neighbors)
    {
      ...
      const auto * sideset_pairs = _study.getInternalSidesets(elem);
      ...
      for (const auto & side_bnd_ids_pair : *sideset_pairs)
      {
        // A side on elem that has an internal sideset
        const auto side = side_bnd_ids_pair.first;

        // First, see if this side has our point
        const auto find_side = std::find(sides.begin(), sides.end(), side);
        if (find_side == sides.end())
        {
          debugRay("      Side ", side, " doesn't contain the the point");
          traceAssert(!sidePtrHelper(elem, side)->contains_point(_intersection_point),
                      "Side contains point");
          continue;
        }

        // The outward normal for this side
        const auto & side_normal = side_normals[std::distance(sides.begin(), find_side)];

        // Side contains our point and isn't parallel to the ray
        if (std::abs(side_normal * ray->direction()) > TRACE_TOLERANCE)
        {
          // TODO: this could likely be optimized
          temp_extrema.invalidate();
          withinExtremaOnSide(elem, _intersection_point, side, _dim, temp_extrema);

          possiblyAddToBoundaryElems(elem, side, side_bnd_ids_pair.second, temp_extrema);
        }
        else
          debugRay("      Side ", side, " contains the point but is parallel");
      }
    }
  }
  ...
  {
    const auto * current_elem_sideset_pairs = _study.getInternalSidesets(_current_elem);
    if (current_elem_sideset_pairs)
      for (const auto & side_bnd_ids_pair : *current_elem_sideset_pairs)
        if (side_bnd_ids_pair.first == _incoming_side)
        {
          ...
          possiblyAddToBoundaryElems(_current_elem,
                                     side_bnd_ids_pair.first,
                                     side_bnd_ids_pair.second,
                                     _dim != 1 ? _intersected_extrema : temp_extrema);
          break;
        }

    const auto * last_elem_sideset_pairs = _study.getInternalSidesets(_last_elem);
    if (last_elem_sideset_pairs)
      for (const auto & side_bnd_ids_pair : *last_elem_sideset_pairs)
        if (side_bnd_ids_pair.first == _intersected_side)
        {
          possiblyAddToBoundaryElems(
              _last_elem, side_bnd_ids_pair.first, side_bnd_ids_pair.second, _intersected_extrema);
          break;
        }
  }
                                   ...
                                   const Elem *& boundary_elem)
{
  ...
  for (const auto & neighbor_info : neighbors)
  {
    // This will be false when we have an edge neighbor that isn't a neighbor at _intersection_point
    if (!neighbor_info._valid)
      continue;
...
TraceRay::trace(const std::shared_ptr<Ray> & ray)
{
  ...
  // Invlidate this up front because it's copied immedtiately into _last_intersected_extrema
  ...
  do
  {
    ...
    if (!_last_intersected_extrema.atExtrema())
    {
      ...
      debugRay("Didn't hit vetex or edge: doing normal exits elem check");
    ...
    // on the previous intersection and didn't bother to do a regular exit check on _current_elem
    if (!_exits_elem)
    {
      debugRay("Moving through neighbors");

      // The element we want moveThroughNeighbors() to try last. That is - if all others fail,
      // this is the last resort. If we have a last intersection that tells us we're going through
      // a vertex or edge, we probably won't go back to that elem. If we don't, it means that
      // we failed the exitsElem() check on _current_elem and probably won't return to it either.
      ...
      // Without preivous vertex/edge information, let's try to find some. We could just do
      ...
      else
      {
        ...
        if (neighbors->empty())
        {
          failTrace("Could not find neighbors to move through", _study.tolerateFailure());
          return;
        }
      }
    ...
    // distance becuse in this case we're close enough to the intersection point
    ...
    // Past the max distance - need to remove the additional distance we travelled,
    ...
    else if (ray->distance() > max_distance)
    {
      ...
      // The distance past the max distance we have travelled
    ...
    // If the mesh has active elements of the same level, just grab the neighbor directly. In this
    // case, we can avoid a call to active() on the neighbor in getActiveNeighbor(), which can be
    // expensive depending on caching.
    ...
    if (neighbor)
    {
      ...
      if (_study.hasSameLevelActiveElems())
      {
        // If the subdomain hasn't changed, we can guarantee n_sides is the same as _current_elem.
        // Prefer this because neighbor->n_sides() is an expensive virtual. Even better,
        // subdomain_id() isn't a virtual!
    ...
    else
    {
      ...
      else if (ray->trajectoryChanged())
      {
        ...
        failTrace("Ray missing action (not killed or changed) on the boundary");
    }
  ...
  failTrace("Could not find an intersection", _study.tolerateFailure());
}
...
TraceRay::onSubdomainChanged(const std::shared_ptr<Ray> & ray, const bool same_ray)
{
  ...
  if (_has_ray_kernels)
  {
    ...
    // Call subdomainSetup to get new kernels etc
    _study.subdomainSetup(_current_subdomain_id, _tid, ray->id());
...
std::string
TraceRay::failTraceMessage(const std::string & reason)
{
  std::stringstream oss;
  oss << "Ray on processor " << _pid << " failed to trace\n\n";
  oss << reason << "\n\n";
  oss << ((*_current_ray))->getInfo(&_study) << "\n";
...
void
TraceRay::failTrace(const std::string & reason, const bool warning)
{
  const auto message = failTraceMessage(reason);
                           ...
                           const Point & point)
{
  ...
  traceAssert(elem->get_node_index(vertices.second) != libMesh::invalid_uint,
              "Doesn't contain vertex");
  traceAssert(isWithinSegment((Point)*vertices.first, (Point)*vertices.second, point),
...
TraceRay::onSegment(const std::shared_ptr<Ray> & ray)
{
  ...
  if (_intersected_side != RayTracingCommon::invalid_side)
  {
    if (_intersected_extrema.atExtrema())
      ...
      traceAssert(
          sidePtrHelper(_current_elem, _intersected_side)->contains_point(_intersection_point),
          "Intersected point is not on intersected side");
  ...
  if (_incoming_side != RayTracingCommon::invalid_side)
  {
    traceAssert(sidePtrHelper(_current_elem, _incoming_side)->contains_point(_incoming_point),
...
TraceRay::onBoundary(const std::shared_ptr<Ray> & ray, const bool external)
{
  ...
  for (RayBoundaryConditionBase * rbc : _on_boundary_ray_bcs)
  {
    ...
    for (MooseIndex(_boundary_elems.size()) bnd_elems_i = 0; bnd_elems_i < _boundary_elems.size();
         ++bnd_elems_i)
      if (rbc->hasBoundary(_boundary_elems[bnd_elems_i]._bnd_id))
        _on_boundary_apply_index.push_back(bnd_elems_i);

    traceAssert(!_on_boundary_apply_index.empty(), "Must not be empty");

    // Apply the RayBC on each of the relavent boundary elements
    for (const auto bnd_elems_i : _on_boundary_apply_index)
    {
      auto & bnd_elem = _boundary_elems[bnd_elems_i];
      _current_elem = bnd_elem._elem;
      _current_bnd_id = bnd_elem._bnd_id;
      _intersected_side = bnd_elem._side;
      _intersected_extrema = bnd_elem._extrema;
      _current_subdomain_id = _current_elem->subdomain_id();
  ...
  if (external && !ray->trajectoryChanged() && ray->shouldContinue())
  {
    ...
    if (!_on_boundary_ray_bcs.empty())
    {
      ...
      for (const RayBoundaryConditionBase * rbc : _on_boundary_ray_bcs)
        for (const auto & bnd_elem : _boundary_elems)
          if (rbc->hasBoundary(bnd_elem._bnd_id))
            oss << "  " << rbc->type() << " '" << rbc->name() << "' on boundary "
                << bnd_elem._bnd_id << " (" << _mesh.getBoundaryName(bnd_elem._bnd_id) << ")\n";
      oss << "\n";
    }
    bool output_header = false;
    for (std::size_t i = 0; i < _boundary_elems.size(); ++i)
    {
      const auto bnd_id = _boundary_elems[i]._bnd_id;
      bool found = false;
      for (const RayBoundaryConditionBase * rbc : _on_boundary_ray_bcs)
