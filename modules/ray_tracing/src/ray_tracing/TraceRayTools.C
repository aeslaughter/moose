...
)
{
  ...
  // the tolerance by hmax. If an intersection is found, the resulting interscection distance is
...
)
{
  ...
  // to invalidate it
  if (intersects && ((intersected_extrema.first == v00 && intersected_extrema.second == v11) ||
                     (intersected_extrema.first == v11 && intersected_extrema.second == v00)))
...
template <typename T>
bool
withinEdgeTempl(const Elem * elem, const Point & point, ElemExtrema & extrema)
{
  mooseAssert(elem->has_affine_map() ? elem->contains_point(point) : true,
              "Elem doesn't contain point");
  mooseAssert(extrema.isInvalid(), "Should be invalid");

  for (int e = 0; e < T::num_edges; ++e)
    if (isWithinSegment(
            elem->point(T::edge_nodes_map[e][0]), elem->point(T::edge_nodes_map[e][1]), point))
    {
...
bool
withinEdge(const Elem * elem, const Point & point, ElemExtrema & extrema)
{
  switch (elem->type())
  {
    case HEX8:
      return withinEdgeTempl<Hex8>(elem, point, extrema);
    case TET4:
      return withinEdgeTempl<Tet4>(elem, point, extrema);
    case PYRAMID5:
      return withinEdgeTempl<Pyramid5>(elem, point, extrema);
    case PRISM6:
      return withinEdgeTempl<Prism6>(elem, point, extrema);
    case HEX20:
    case HEX27:
      return withinEdgeTempl<Hex8>(elem, point, extrema);
    case TET10:
...
bool
isWithinSegment(const Point & segment1, const Point & segment2, const Point & point)
{
  ...
  return isWithinSegment(segment1, segment2, segment_length, point);
}
                ...
                const Point & point)
{
  ...
  if (diff1 * diff2 > TRACE_TOLERANCE * segment_length)
    return false;

  return std::abs(diff1.norm() + diff2.norm() - segment_length) < TRACE_TOLERANCE * segment_length;
}
