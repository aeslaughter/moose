...
{
  if (!ray)
    mooseError(_error_prefix, ": Cannot move a null Ray to the buffer.");
  ...
  if (_verify_rays) // only guard with this in optimized modes
    ...
    verifyRay(ray,
              tid,
              verify_ray_error_prefix = "" ? _verify_ray_error_prefix : verify_ray_error_prefix);
                                  ...
                                  std::string verify_ray_error_prefix)
{
  ...
  _parallel_ray_study->moveWorkToBuffer(rays, tid);
}
