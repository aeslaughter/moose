...
RayTracingMeshOutput::validParams()
{
  ...
  params.addParam<bool>("output_data_nodal",
                        false,
                        "Whether or not to output the Ray's data in a nodal since, in which the "
...
RayTracingMeshOutput::output(const ExecFlagType & type)
{
  ...
  // Done with these
  // We don't necessarily need to create a new mesh every time, but it's easier than checking if the
  // Rays have changed from last time we built a mesh
  _es->clear();
  _es = nullptr;
  _sys = nullptr;
  _segment_mesh->clear();
...
RayTracingMeshOutput::buildIDMap()
{
  ...
  // Fill all of my local Ray maxes to be sent to processor 0
...
RayTracingMeshOutput::buildSegmentMesh()
{
  ...
  for (const auto & trace_data : _study.getCachedTraces())
  {
    ...
    // The has more of a trace after this one
    if (!trace_data._last)
    {
  ...
  // Functor that takes in a set if incoming node IDs from a processor and decides on an owner.
...
RayTracingMeshOutput::buildBoundingBoxes()
{
  ...
  // Avoid creating these again if we're already built them and everyone's are the same
  if (!_inflated_bboxes.empty())
  {
    unsigned int changed = previous_bbox != _bbox;
    _communicator.sum(changed);
    if (changed == 0)
      return;
  }
