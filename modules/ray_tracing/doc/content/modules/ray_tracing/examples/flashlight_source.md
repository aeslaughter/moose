...
A "flashlight" point source is an anisotropic point source that emits in a cone about a direction $\hat{\Omega}$. The cone is defined by the the vector along the cone axis and half of the opening angle in degrees.

The [ray_tracing/index.md] is utilized for this example to discretize the angular integration. Angular quadrature is sampled within the cone and a [Ray.md] generated for each direction. These [Rays](Ray.md) contribute to PDEs solved on the domain via line sources.

## Example

Consider a problem on a two-dimensional mesh in the physical domain $D \in [0, 5] \times [0, 5]$. It will contain a reaction term formed by the [Reaction.md] kernel and a diffusion term formed by the [Diffusion.md] kernel. The anisotropic point source is located at $(1, 1.5)$ that emits a source in a cone $2 \degree$ about  $\hat{\Omega} = (2 / \sqrt{5}, 1 / \sqrt{5})$. The right boundary will specularly reflect the point source.
...
- [!param](/UserObjects/ConeRayStudy/half_cone_angles) - Half of the opening angle of the cone in degrees, defined as $2 \degree$
