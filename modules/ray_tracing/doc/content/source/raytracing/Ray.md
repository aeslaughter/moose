...
This guarantees that [RayTracingStudy.md] objects, all [RayBC](RayBCs/index.md) objects, and all [RayKernel](RayKernels/index.md) objects will have access to the data that they need on all `Ray`s that are traced. This registration devises an index into the `Ray` data and auxiliary data and sizes that all `Ray` data and auxiliary data should be sized to. For more information, see [RayTracingStudy.md#ray-data-registration].
