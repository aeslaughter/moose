...
- `_current_segment_end` - The end point of the current segment being operated on. This is not necessarily on the element periphery in the case that a [Ray.md] has en
...
- If it has reached the user-set end point (when the [Ray.md] trajectory is set via `setStartEnd()`). In this specific case, you can tell if the [Ray.md] has hit its end point by `currentRay()->atEnd()`.
...
- Only one trajectory change can be called by all RayKernels on a single segment.
...
By registering the data, as long as you are appropriately sizing your generated [Rays](Ray.md) (via `RayTracingStudy::rayDataSize()` and `RayTracingStudy::rayAuxDataSize()`, done automatically when generating a [Ray.md] within a RayKernel or RayBC), the data can be obtained by `currentRay().data()` and `currentRay().auxData()`.

In the case of the [IntegralRayKernel.md], the [Ray.md] data is accessed as such:

!listing modules/ray_tracing/src/ray_kernels/IntegralRayKernel.C re=void\sIntegralRayKernel::computeIntegral.*?^}
